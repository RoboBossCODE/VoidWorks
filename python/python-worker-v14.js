const RUNTIME_BASE = new URL("./runtime/", self.location.href).href;
...
async function boot() {
  try {
    ...
    // Self-hosted alongside this worker: no external CDN.
    importScripts(RUNTIME_BASE + "pyodide.js");
    ...
    send("bootError", {
      message:
        "The local Python runtime is not installed yet, or one of its files is missing.\n\n" +
        msg +
        "\n\nRun the 'Set up Voidworks Python runtime' GitHub Action once, then reload this page."
    });
  }
}
