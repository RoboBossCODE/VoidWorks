...
function makeWorker(){
  ...
  worker = new Worker("python-worker-v16.js?build=16", { type: "module" });
...
function onWorkerMessage(e){
  ...
  switch(m.type){
    ...
    case "pythonError":
      showFriendlyError(m.parsed || {
        type:"PythonError",
        message:"Python execution failed.",
        raw:cleanTraceback(m.message)
      });
      break;
    case "runStart":
      running = true;
...
function showFriendlyError(err){
  const box = $("friendlyError");
  $("errorType").textContent = err.type || "PythonError";
  $("errorMessage").textContent = err.message || "Python execution failed.";
  $("errorHint").textContent = err.hint || "Check the code around the reported line.";

  const location = $("errorLocation");
  if(err.line){
    location.hidden = false;
    $("errorLineLabel").textContent = `Line ${err.line}${err.column ? ` · Column ${err.column}` : ""}`;

    let source = err.source || "";
    if(err.column && source){
      const caretPos = Math.max(0, err.column - 1);
      source += "\n" + " ".repeat(caretPos) + "^";
    }
    $("errorSource").textContent = source || "(source line unavailable)";
  }else{
    location.hidden = true;
    $("errorSource").textContent = "";
  }

  $("errorTrace").textContent = err.raw || "";
...
$("dismissError").addEventListener("click", hideFriendlyError);
...
$("clearBtn").onclick=()=>{
  hideFriendlyError();
  consoleEl.innerHTML='<span class="muted">Output cleared.</span>';
};
