...
async function installPackage(name) {
  try {
    send("packageStatus", {status:"working", message:`Installing ${name}…`});
    await pyodide.loadPackage("micropip");
    pyodide.globals.set("__vw_package", name);
    await pyodide.runPythonAsync(`
import micropip
await micropip.install(__vw_package)
`);
    pyodide.globals.delete("__vw_package");
    send("packageStatus", {status:"ok", message:`Installed ${name}`, name});
  } catch (err) {
    send("packageStatus", {status:"error", message:String(err && err.stack ? err.stack : err)});
  }
}
